...
import { useImageVulnerabilitiesColumn } from "@/components/tables/images/vulnerability-image-tags.column";
import { DataTable } from "@/components/tables/table-component";
import { Button } from "@/components/ui/button";
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@/components/ui/dropdown-menu";
import { ChevronDown, Search } from "lucide-react";
import { useMemo, useState } from "react";
import { ArrowsSort, BrandGoogleBigQuery } from "tabler-icons-react";
import AdvancedSearchQueryBuilder from "../querybuilder/AdvancedSearchQueryBuilder";
import { VulnerabilitiesData } from "./mockdata";

export default function VulnerabilitiesTabContent({ vulnerabilitiesData }: { vulnerabilitiesData: VulnerabilitiesData[] }) {
    ...
    const columns = useImageVulnerabilitiesColumn()
    ...
    const filteredData = useMemo(() => {
        ...
        if (search.trim()) {
            data = data.filter(row => row.id.toLowerCase().includes(search.toLowerCase()));
        }
        return data;
    }, [search, vulnerabilitiesData]);
    ...
    return (
        <div className="h-full flex flex-col">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <div className="flex rounded-md overflow-hidden border border-[#363636] bg-[#282828] h-[34px] text-xs cursor-pointer">
                                ...
                                >
                                    <span>{versionOptions.find(o => o.value === version)?.label}</span>
                        ...
                        <DropdownMenuContent align="end">
                            {versionOptions.map(opt => (
                                <DropdownMenuItem key={opt.value} onClick={() => { setVersion(opt.value); }}>
                                    {opt.label}
                                </DropdownMenuItem>
                            ))}
                        </DropdownMenuContent>
            ...
            <div className="flex-grow flex flex-col bg-[#1B1B1B] rounded-lg w-full">
                <DataTable
                    isLoading={false}
                    data={filteredData}
                    columns={columns}
                                ...
                                <div className="w-full h-[220px] rounded-b-lg flex gap-3">
                                    ...
                                    <div className="flex-1 flex flex-col rounded justify-around border">
                                        ...
                                        <div className="flex justify-between mx-6">
                                            ...
                                            <div className="text-sm">{vuln.fixedVersion || '-'}</div>
                                        </div>
                                    </div>
                                </div>
                                <div className="">
